...
LPINT merge(LPINT v1, INT *n1, LPINT v2, INT n2);
...
INT _tmain(INT argc, LPTSTR argv[]) {
	...
	for (INT i = 0; i < argc - 2; i++) {
		...
		h[i] = CreateThread(NULL, 0, readerThread, (PVOID)&p[i], 0, NULL);
#ifndef SKIP_READING
		Sleep(500);
#endif
	}
	...
	while (nThreads > 0) {
		...
		if (status >= WAIT_OBJECT_0 && status < WAIT_OBJECT_0 + nThreads) {
			...
			h[i] = h[nThreads - 1];
			nThreads--;
			/* Merge the current array with the one returned by the thread */
			values = merge(values, &n, p[i].values, p[i].n);
		}
	}
...
LPINT merge(LPINT v1, INT *n1, LPINT v2, INT n2) {
	LPINT v = (LPINT) malloc((*n1 + n2) * sizeof(INT));
	INT i = 0, j = 0, k = 0;
